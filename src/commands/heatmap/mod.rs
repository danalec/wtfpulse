...
use crate::tui::app::{App, TimePeriod};
    ...
    match key.code {
        KeyCode::Char('h') => {
            app.dashboard_period = match app.dashboard_period {
                TimePeriod::Today => TimePeriod::All,
        ...
        KeyCode::Char('l') => {
            app.dashboard_period = match app.dashboard_period {
                ...
                TimePeriod::All => TimePeriod::Today,
...
fn fetch_heatmap(app: &App) {
    ...
    crate::tui::app::spawn_fetch_heatmap(app.client.clone(), app.tx.clone(), period_str);
}
...
fn render_keyboard(f: &mut Frame, app: &App, area: Rect) {
    ...
    // Keyboard size is roughly 60x15
    let kbd_width = 60;
    let kbd_height = 15;
